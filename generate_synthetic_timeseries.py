...
def generate_synthetic_timeseries(length=3000, num_incidents=10):
    t = np.arange(length)
    # Base signal (e.g., daily seasonality)
    ...
    for _ in range(num_incidents):
        ...
        # Length of the precursor (early symptoms before the actual failure)
        precursor_len = 15

        if inc_type == "spike":
            # Precursor: Rising trend and higher noise
            for i in range(precursor_len):
                idx = start - precursor_len + i
                series[idx] += (i / precursor_len) * 5 + np.random.normal(0, 1)
            # Actual incident
            spike_power = random.uniform(10, 25)
            series[start:end] += spike_power + np.random.normal(0, 2, end - start)

        elif inc_type == "variance_drop":
            # Precursor: Slow signal drop (e.g., sensor freezing)
            for i in range(precursor_len):
                idx = start - precursor_len + i
                series[idx] -= (i / precursor_len) * 4
            # Actual incident
            series[start:end] = np.random.normal(0, 0.5, end - start)

        # Remove used indices with a buffer to prevent overlapping
        available_indices = [idx for idx in available_indices if idx < start - 30 or idx > end + 30]
